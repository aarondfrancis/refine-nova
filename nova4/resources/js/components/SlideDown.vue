  ...
  methods: {
    ...
    beforeLeave: function () {
      this.$refs.wrapper.style.height =
        this.outerHeight(this.$refs.wrapper) + 'px'
      this.$refs.wrapper.style.overflowY = 'hidden'
    },

    enter: function (arg1, arg2) {
      let done = false ? arg1 : arg2
    ...
    animate: function (growing, start, done) {
      ...
      let original = growing
        ? 0
        : this.outerHeight(this.$refs.wrapper.children.item(0))
      let destination = growing
        ? this.outerHeight(this.$refs.wrapper.children.item(0))
        : 0
