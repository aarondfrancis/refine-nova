        ...
        <div class=''>
            <div v-for="(group, index) in groupedBlueprint" :key="index">
                <div class='border rounded-lg shadow border-60'>
                    <div class='border-b border-50 py-3 pl-2 bg-white w-full' :class='{"rounded-t-lg": index === 0}' v-for="(criterion, index) in group" :key="criterion.uid">
                        <renderless-condition v-bind="conditionFor({id: criterion.condition_id, ...criterion})" v-slot="{ switchClause }">
                            <criterion
              ...
              "
                    ...
                    <button @click="insertCriterion(group[group.length - 1].position)"
