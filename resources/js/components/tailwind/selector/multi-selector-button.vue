<template>
  <div
    ...
    tabindex="0"
  >
    <span v-if="selectedOptions.length === 0" class="refine-multi-selector-button-placeholder"> Choose an option </span>
    <span v-else v-for="{ id, display } in selectedOptions" class="refine-multi-selector-button-selected" :key="id">
      {{ display }}
      <span class="refine-multi-selector-button-deselect-icon-wrapper" @click.prevent="$emit('deselect-option', id)">
        <svg
